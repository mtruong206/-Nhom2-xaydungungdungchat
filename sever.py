    ...
    def run_dh_exchange(self, role='server', host='127.0.0.1', port=65432):
        ...
        try:
            if role == 'server':
                ...
                conn, addr = s.accept()
                print(f"[Trường Server] Đã có Client kết nối từ {addr}")
...
if __name__ == "__main__":
    mang = TruongNetwork()
    # Chạy file này với vai trò SERVER
    conn, key = mang.run_dh_exchange(role='server')
    
    if key:
        print(f" Khóa chung của SERVER tính ra là: {binascii.hexlify(key).decode()}")